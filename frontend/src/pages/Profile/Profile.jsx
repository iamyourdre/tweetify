...
const Profile = () => {
  ...
  return (
    <div className='min-h-screen border-r border-gray-600'>
      {user && 
        <>
          <ProfileHeader user={user} isFollowing={isFollowing} followUser={followUser} unfollowUser={unfollowUser} following={following} followers={followers} meId={me._id} logout={logout} />
...
const ProfileHeader = ({ user, isFollowing, followUser, unfollowUser, following, followers, meId, logout }) => {
  return (
    <div className="flex items-start gap-4 px-4 py-16 lg:px-10 bg-base-300">
      ...
      <div className="flex-1 flex justify-end items-end">
        {user._id !== meId ? (
          ...
          <Button text="Logout" addClass="btn-outline btn-error" onClick={logout} />
        )}
      </div>
    </div>
  );
